...
<head>
    ...
    <meta name="description"
        content="Jadwal, tugas, kelompok, dan pengumuman kelas dalam satu aplikasi. Mulai Rp150.000 per semester untuk satu kelas.">
    ...
    <style>
        :root {
            ...
            --kertas: #f6f5f1;
            --kunyit: #f6b52a;
            /* Same accent, darkened just enough to hold up as large text on the paper surfaces. */
            --kunyit-tua: #c4841a;
            --redup: #5b6577;
            --garis: #ddd9cf;
        }
...
<body class="h-full bg-[var(--kertas)] font-sans text-[var(--tinta)] antialiased">
    ...
    <header id="kepala" class="kepala fixed inset-x-0 top-0 z-40 border-b border-[var(--garis)]">
        <div class="mx-auto flex h-16 w-full max-w-6xl items-center gap-3 px-5 sm:gap-7 sm:px-8">
    ...
    <main id="atas" class="pt-16">
        ...
        <section class="mx-auto w-full max-w-6xl px-5 pb-16 sm:px-8 sm:pb-24">
            <div class="border-t border-[var(--garis)] pt-10">
                <p class="text-center text-sm text-[var(--redup)] lg:text-left">Hitung patungannya dulu</p>
                <div id="kalkulator"
                    class="mt-5 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,22rem)] lg:items-start">
                    <div class="text-center lg:text-left">
                        <div class="flex flex-wrap items-baseline justify-center gap-x-4 gap-y-2 lg:justify-start">
                            <span id="harga-terpilih" class="angka text-3xl font-bold sm:text-4xl">Rp150.000</span>
                        ...
                        <div class="mt-7 flex flex-wrap items-end justify-center gap-x-3 gap-y-1 lg:justify-start">
                            <span class="angka text-6xl font-bold leading-none text-[var(--kunyit-tua)] sm:text-7xl"
                                id="per-orang">Rp3.948</span>
                        ...
                        <input id="jumlah" name="jumlah" type="range" min="10" max="60" step="1"
                            value="38" class="geser mx-auto mt-4 block w-full max-w-md lg:mx-0" aria-describedby="per-orang">
                        <div class="mx-auto mt-1 flex w-full max-w-md justify-between text-xs text-[var(--redup)] lg:mx-0">
        ...
        <section id="isinya" class="border-y border-[var(--garis)] bg-white">
            <div class="mx-auto w-full max-w-6xl px-5 py-16 sm:px-8 sm:py-20">
                ...
                {{-- Two columns of six items each, so no card is left with a half-empty box beside a full one. --}}
                <div class="mt-10 grid gap-5 sm:grid-cols-2">
                    @foreach ([['Akademik', 'Informasi'], ['Jadwal', 'Pengelolaan']] as $kolom)
                        <div class="flex flex-col gap-5">
                            @foreach ($kolom as $grup)
                                <div class="flex-1 rounded-2xl border border-[var(--garis)] bg-[var(--kertas)] p-6 sm:p-7">
                                    <h3 class="text-xs font-semibold uppercase tracking-wider text-[var(--kunyit-tua)]">
                                        {{ $grup }}</h3>
                                    <ul class="mt-5 space-y-4">
                                        @foreach ($menu[$grup] as [$ikon, $judul, $keterangan])
                                            <li class="flex gap-3.5">
                                                <span
                                                    class="flex size-9 shrink-0 items-center justify-center rounded-lg border border-[var(--garis)] bg-white">
                ...
                <div class="mt-5 rounded-2xl border border-[#f7dc95] bg-[#fff6d8] p-6 sm:p-8">
                    <h3 class="text-lg font-bold tracking-tight">Rapi karena perannya jelas</h3>
                    <div class="mt-6 grid gap-6 sm:grid-cols-3 sm:gap-8">
                        <div>
                            <x-heroicon-o-eye class="size-6 text-[var(--kunyit-tua)]" />
                            <p class="mt-3 text-[15px] font-semibold leading-snug">Mahasiswa melihat dan berbagi</p>
                            <p class="mt-1.5 text-sm leading-relaxed text-[var(--redup)]">Admin kelas yang mengatur
                                jadwal, tugas, dan pengumuman.</p>
                        </div>
                        <div>
                            <x-heroicon-o-arrow-up-tray class="size-6 text-[var(--kunyit-tua)]" />
                            <p class="mt-3 text-[15px] font-semibold leading-snug">Unggah hanya oleh admin kelas</p>
                            <p class="mt-1.5 text-sm leading-relaxed text-[var(--redup)]">
                                Mengunggah file dan gambar hanya bisa dilakukan admin kelas, jadi tidak ada yang iseng atau mengirim spam.
                            </p>
                        </div>
                        <div>
                            <x-heroicon-o-lock-closed class="size-6 text-[var(--kunyit-tua)]" />
                            <p class="mt-3 text-[15px] font-semibold leading-snug">Kelompok bisa dikunci</p>
                            <p class="mt-1.5 text-sm leading-relaxed text-[var(--redup)]">Kalau pembagiannya sudah
                                final, tidak ada lagi yang mengubah diam-diam.</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="mt-12 grid gap-6 lg:grid-cols-2">
                    <div
                        ...
                        <a href="{{ $wa }}" target="_blank" rel="noopener"
                            class="mt-auto flex w-full items-center justify-center rounded-xl bg-[var(--tinta)] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#0f1a2e]">
                            Ambil promo ini
                        </a>
                    </div>
                    ...
                    <div class="flex flex-col rounded-2xl border border-white/20 p-7 sm:p-9">
                        ...
                        <a href="{{ $wa }}" target="_blank" rel="noopener"
                            class="mt-auto flex w-full items-center justify-center rounded-xl border border-white/30 px-5 py-3 text-sm font-semibold text-white transition hover:bg-white/10">
                            Langganan paket ini
                        </a>
                    </div>
                </div>
        ...
        <section class="mx-auto w-full max-w-6xl px-5 py-16 sm:px-8 sm:py-20">
            <div class="grid gap-10 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
                ...
                <div class="flex flex-col gap-3 sm:items-end">
                    <a href="{{ $wa }}" target="_blank" rel="noopener"
                        class="inline-flex items-center justify-center gap-2.5 rounded-xl bg-[#1f7a63] px-6 py-3.5 font-semibold text-white transition hover:bg-[#1a6653] sm:w-64">
                        <x-ui.whatsapp-icon class="size-5" />
                        Chat admin
                    </a>
                    <a href="mailto:{{ $email }}"
                        class="inline-flex items-center justify-center gap-2.5 rounded-xl border border-[var(--garis)] bg-white px-6 py-3.5 font-semibold transition hover:bg-[var(--kertas)] sm:w-64">
    ...
    <footer class="border-t border-[var(--garis)] bg-white">
        <div class="mx-auto w-full max-w-6xl px-5 py-12 sm:px-8 sm:py-16">
            <div class="grid gap-10 md:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)_minmax(0,1fr)] md:gap-8">
                <div>
                ...
                <div>
                    ...
                    <ul class="mt-4 space-y-2.5 text-sm font-medium">
                        <li>
                            <a href="{{ $wa }}" target="_blank" rel="noopener"
                                class="inline-flex items-center gap-2.5 transition hover:text-[var(--kunyit-tua)]">
                                <x-ui.whatsapp-icon class="size-4 shrink-0 text-[var(--redup)]" />
                                Admin Kelas KampusKu
                            </a>
                        </li>
                        <li>
                            <a href="mailto:{{ $email }}"
                                class="inline-flex items-center gap-2.5 transition hover:text-[var(--kunyit-tua)]">
                                <x-heroicon-o-envelope class="size-4 shrink-0 text-[var(--redup)]" />
                                {{ $email }}
                            </a>
                        </li>
                    </ul>
    ...
    <script>
        ...
        (() => {
            ...
            const hitung = () => {
                const harga = Number(paket.find((pilihan) => pilihan.checked)?.value ?? 150000);
