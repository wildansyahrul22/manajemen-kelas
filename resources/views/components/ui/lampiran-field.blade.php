{{--
    File picker for a form object using WithLampiran: the stored attachments (with a "hapus" tick),
    the input itself (several files at once, uploaded straight away with a progress bar), the newly
    picked files (cancellable), and the validation errors or the limits hint.

    `model` is the class with the LAMPIRAN_* limits (Informasi / Tugas); `sebutan` is how that
    record is called in the hint ("2 file per informasi"). Files too big for the server are refused
    in the browser before any upload starts.
--}}
@props(['form', 'model', 'sebutan', 'label' => 'Lampiran (gambar/file)'])
@php
...
>
    <label for="form-lampiran" class="mb-1.5 block text-sm font-medium text-slate-700">{{ $label }}</label>
    ...
    <input
        ...
        accept="{{ collect($model::LAMPIRAN_EKSTENSI)->map(fn ($ext) => '.'.$ext)->join(',') }}"
        @class([
    ...
    @else
        <p class="mt-1.5 text-xs text-slate-500">
            Bisa pilih beberapa file sekaligus. Maks. {{ \App\Support\BatasUnggah::mb($perFile) }} MB per file, {{ $model::LAMPIRAN_MAKS_JUMLAH }} file per {{ $sebutan }}; format {{ strtoupper(implode(', ', $model::LAMPIRAN_EKSTENSI)) }}. File hanya bisa dibuka oleh anggota kelas.
        </p>
    @endif
</div>
