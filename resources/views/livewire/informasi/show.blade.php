<div>
    ...
    <x-ui.card class="max-w-3xl">
        ...
        @if ($informasi->link || $informasi->hasLampiran())
            @php $gambar = $informasi->lampiran->filter(fn ($lampiran) => $lampiran->isImage()); @endphp
            <div class="mt-5 space-y-3">
                @if ($gambar->isNotEmpty())
                    <div @class(['grid gap-3', 'grid-cols-1' => $gambar->count() === 1, 'grid-cols-2' => $gambar->count() > 1])>
                        @foreach ($gambar as $lampiran)
                            <a href="{{ route('informasi.lampiran', [$informasi, $lampiran]) }}" target="_blank" rel="noopener" wire:key="gambar-{{ $lampiran->id }}" class="block overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
                ...
                @foreach ($informasi->lampiran as $lampiran)
                    <a href="{{ route('informasi.lampiran', [$informasi, $lampiran]) }}" target="_blank" rel="noopener" wire:key="lampiran-{{ $lampiran->id }}" class="flex items-center gap-3 rounded-xl border border-slate-200 px-4 py-3 text-sm transition hover:border-primary-300 hover:bg-slate-50">
                        <span class="flex size-9 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-500">
                            @if ($lampiran->isImage())<x-heroicon-o-photo class="size-5" />@else<x-heroicon-o-document-arrow-down class="size-5" />@endif
                        </span>
                        <span class="min-w-0 flex-1">
                            <span class="block truncate font-medium text-slate-800">{{ $lampiran->nama }}</span>
                            <span class="block text-xs text-slate-500">Lampiran · {{ strtoupper($lampiran->ekstensi()) }}{{ $lampiran->ukuranTerbaca() ? ' · '.$lampiran->ukuranTerbaca() : '' }} · klik untuk {{ $lampiran->isImage() ? 'membuka' : 'mengunduh' }}</span>
                        </span>
