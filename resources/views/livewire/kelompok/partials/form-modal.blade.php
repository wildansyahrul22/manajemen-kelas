<x-ui.modal model="showForm" :title="$form->kelompok ? 'Edit Kelompok' : 'Buat Kelompok'" max-width="max-w-2xl" loading="openCreate, openEdit">
    <form id="form-kelompok" wire:submit="save" class="space-y-4">
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <x-ui.combobox label="Kategori kelompok" name="form.kategori_kelompok_id" wire:model.live="form.kategori_kelompok_id" :options="$this->kategoriOptions" placeholder="Pilih kategori" required
                :hint="$this->kategoriOptions->isEmpty() ? 'Belum ada kategori. Buat dulu di menu Kategori Kelompok.' : null" />
        ...
        <div x-data="{ cari: '' }" wire:loading.class="opacity-60" wire:target="form.kategori_kelompok_id" class="transition-opacity">
            ...
            <div class="scrollbar-thin max-h-56 divide-y divide-slate-100 overflow-y-auto rounded-xl border border-slate-200">
                @if ($form->kategori_kelompok_id === '')
                    <p class="px-4 py-6 text-center text-sm text-slate-400">Pilih kategori kelompok terlebih dahulu untuk melihat mahasiswa yang tersedia.</p>
                @else
                    {{-- Only query the student list while the modal is open. --}}
                    @forelse ($showForm ? $this->mahasiswaOptions : [] as $mahasiswa)
                        <label
                    ...
                    @empty
                        <p class="px-4 py-6 text-center text-sm text-slate-400">Semua mahasiswa sudah tergabung dalam kelompok pada kategori ini.</p>
                    @endforelse
                @endif
            </div>
            @if ($form->kategori_kelompok_id !== '')
                <p class="mt-1.5 text-xs text-slate-500">Mahasiswa yang sudah punya kelompok pada kategori ini tidak ditampilkan.</p>
            @endif
