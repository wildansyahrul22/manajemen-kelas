...
**URL memakai ULID, bukan id.** Semua yang tampil di URL — halaman detail tugas, mata kuliah, informasi, kelompok, user, serta unduhan lampiran — memakai kolom `ulid` (26 karakter, 80 bit acak) sebagai route key, sehingga tautan tidak bisa ditebak dengan menaikkan angka. `id` auto-increment tetap jadi primary key untuk relasi. Diatur oleh trait `App\Models\Concerns\RoutesByUlid` (di atas `HasUlids` bawaan Laravel); ULID dibuat model saat insert dan diisi untuk data lama oleh migrasi `add_ulid_to_routed_tables`. Ingat menyertakan `ulid` pada `select([...])` / `with('relasi:id,ulid,...')` bila hasilnya dipakai `route()`.

Setiap informasi bisa menyertakan **tautan** (mis. file di Google Drive yang aksesnya dibuka untuk "siapa saja yang memiliki link") — ini cara mahasiswa membagikan gambar/file. Admin kelas dan super admin bisa mengunggah **beberapa lampiran** sekaligus (maks. 2 MB per file, 2 file per informasi — di hosting dijamin oleh `public/.user.ini`: `upload_max_filesize = 3M`, `post_max_size = 8M`; form menampilkan batas efektif server dan menolak file kebesaran sebelum diunggah; disimpan di `storage/app/private/informasi` dan hanya bisa dibuka anggota kelas lewat `/informasi/{ulid}/lampiran/{ulid}`). **Tugas** punya lampiran yang sama (soal, template laporan; maks. 2 file × 2 MB per tugas, disimpan di `storage/app/private/tugas`, dibuka lewat `/tugas/{ulid}/lampiran/{ulid}`) — bagian yang sama dipakai keduanya: trait `HasLampiran`/`LampiranFile` (model), `WithLampiran` (form Livewire), serta komponen Blade `x-ui.lampiran-field` (form) dan `x-ui.lampiran-list` (halaman detail, dengan galeri gambar dan pratinjau PDF).
