...
class ExportExcelTest extends TestCase
{
    public function test_tugas_export_follows_the_active_filters_and_is_formatted(): void
    {
        ...
        $laporan = Tugas::factory()->create(['mata_kuliah_id' => $web->id, 'kategori_kelompok_id' => $kategori->id, 'nama' => 'Laporan 1', 'deskripsi' => "Baris satu\nBaris dua", 'deadline' => '2026-09-20 23:59:00', 'link_pengumpulan' => 'https://forms.gle/abc', 'created_by' => $admin->id]);
        TugasLampiran::factory()->create(['tugas_id' => $laporan->id, 'nama' => 'soal.pdf', 'ukuran' => 2048]);
        ...
        $this->assertSame(['No', 'Nama Tugas', 'Mata Kuliah', 'Dosen', 'Deadline', 'Status', 'Tugas Kelompok (Kategori)', 'Link Pengumpulan', 'Jumlah Lampiran', 'Deskripsi', 'Dibuat Oleh', 'Dibuat Pada'], $rows[5]);
        $this->assertSame(['1', '=SUM(1)', 'Pemrograman Web', 'Dr. Andi', '16 Sep 2026 12:00', 'Segera', null, null, '0', null, 'Rizky', '15 Sep 2026 09:00'], $rows[6]);
        $this->assertSame(['2', 'Laporan 1', 'Pemrograman Web', 'Dr. Andi', '20 Sep 2026 23:59', 'Aktif', 'Project Akhir', 'https://forms.gle/abc', '1', "Baris satu\nBaris dua", 'Rizky', '15 Sep 2026 09:00'], $rows[7]);
        ...
        $this->assertSame(50.0, $sheet->getColumnDimension('J')->getWidth());
        $this->assertTrue($sheet->getStyle('J7')->getAlignment()->getWrapText());

        $spreadsheet = $this->downloadedSpreadsheet($component);
        $this->assertSame(['Tugas', 'Lampiran'], $spreadsheet->getSheetNames());

        $lampiran = $spreadsheet->getSheetByName('Lampiran')->toArray(null, true, true, false);
        $this->assertSame(['No', 'Nama Tugas', 'Mata Kuliah', 'Nama File', 'Ukuran'], $lampiran[5]);
        $this->assertSame(['1', 'Laporan 1', 'Pemrograman Web', 'soal.pdf', '2.0 KB'], $lampiran[6]);
        $this->assertCount(7, $lampiran);
    }
